...
- 首页最终形态 = WaveHome（一屏编排，1320×900 无滚动条）：
  1. Hero 分栏（45/55）：NowPlayingCard（唱盘 + 封面 + 播放键）+ 榜单 Spotlight；
  2. **编辑精选**：非对称马赛克（1 大 + 4 小）；
  3. **排行榜**：紧凑目录（名次 + 名称 + 曲目数，三列两行）；
  4. **每日推荐**：8 张卡横轨；
  5. **最近播放**：紧凑行卡轨。
- 纵向节奏由 `useStageScale` 按视口高度整体缩放（一屏预算实测标定，`scripts/measure-home.cjs` 验收）；区块头 = 标题 + 右侧 hint；动效：播放态 rAF 每帧写一次 `--mo-beat`，封面光晕呼吸；3D 只作用于封面层。
