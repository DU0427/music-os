...
import type { ProviderAccount } from '../../../src/shared/music/providers';
import { fetchAccount } from './auth';
import { getNeteaseSession } from './http';
 ...
 */
export async function openNeteaseLoginWindow(parent: BrowserWindow | null): Promise<ProviderAccount | null> {
  const neteaseSession = getNeteaseSession();
  ...
  void loginWindow.loadURL(LOGIN_URL).catch((error: unknown) => {
  ...
  return new Promise<ProviderAccount | null>((resolve) => {
    let settled = false;

    const finish = async (account: ProviderAccount | null) => {
      if (settled) {
        return;
      }
      settled = true;
