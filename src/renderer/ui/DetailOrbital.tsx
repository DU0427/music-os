...
import { X, Play, Pause, ExternalLink, SkipForward } from 'lucide-react';
...
export default function DetailOrbital({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const accent = useDominantColor(track?.artworkUrl ?? null, '#f5f5f7');
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-5xl px-8 md:px-12 flex flex-col md:flex-row gap-10 md:gap-16 items-center">
            ...
            <div className="flex-shrink-0 flex flex-col items-center">
              <CoreVisualDom size={280} accent={accent} onClick={() => (isPlaying ? pause() : void play())} />
              <motion.div
            ...
            <motion.div className="flex-1 flex flex-col justify-center" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
              <div className="space-y-6 relative">
                <div className="absolute left-[3px] top-3 bottom-3 w-px bg-gradient-to-b from-white/10 via-[var(--mo-accent-ghost)] to-transparent" />

                <div className="relative pl-6">
                  <div className="absolute left-0 top-1.5 w-1.5 h-1.5 rounded-full" style={{ background: 'var(--mo-accent)', boxShadow: `0 0 8px ${withAlpha(accent, 0.8)}` }} />
              ...
              <div className="mt-8 space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-full border border-white/10 hover:bg-white/5 text-white/45 hover:text-white/80 transition-all"
                    onClick={() => useAudioStore.getState().skipNext()}
                    title="跳到队列下一首"
                  >
                    <SkipForward className="w-3 h-3" />
                    <span className="font-mono text-[10px] tracking-wide">下一首</span>
                  </button>
                </div>
              </div>
            </motion.div>
